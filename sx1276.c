...
//mcu_delayms(10);
  ...
  // decia    sizeof(radio_registers) / sizeof(radio_registers_t)
    ...
    case RF_RX_RUNNING:
      ...
      switch(sx1276.Settings.Modem) {
        ...
        case MODEM_LORA: {
          ...
          // ACA HACEMOS QUE SE DIGA EL DATO POR UART
          
          
          int jo;
          if (RxTxBuffer[10] == 0){ // si esto sucede estamos mandando la palabra latitud o longitud 
            
            
            if(RxTxBuffer[2] == 'N'){       // si la palabra enviada es longitud
              uart_write("LONGITUD:");
            }
            if(RxTxBuffer[2] == 'T'){      // si la palabra enviada es latitud
               uart_write("LATITUD:");
            }
            
         }
          else {                 // caso contrario, estamos recibiendo o latitud o longitud. Enviarla x UART
            
            jo=32;
            while (jo>0){
              uart_writec(RxTxBuffer[jo]);
              uart_writec(' ');
              jo--;
            }
          }
          ...
          //  P1OUT |= BIT0;
            ...
            uart_write("RSSI:");
            uart_printhex32(sx1276.Settings.LoRaPacketHandler.RssiValue);
            uart_writec(',');
            
            uart_write("SNR:");
            uart_printhex8(sx1276.Settings.LoRaPacketHandler.SnrValue);
            uart_writec(',');
            
            uart_write("SIZE:");
            uart_printhex32(sx1276.Settings.LoRaPacketHandler.Size);
            uart_writec(',');

            //uart_printhex32(*RxTxBuffer);
            if (*RxTxBuffer==0){
              uart_write("ACK");
              uart_writec('\n');
            } else {
...
__interrupt void ISR_TRA19(void)
{
      while(1);
}
#pragma vector = USCI_A3_VECTOR
__interrupt void ISR_TRA18(void)
{
      while(1);
}
#pragma vector = USCI_B1_VECTOR
__interrupt void ISR_TRA17(void)              //17
{
      while(1);
}
#pragma vector = USCI_A1_VECTOR
__interrupt void ISR_TRA16(void)
{
while(1){
break;
}
...
__interrupt void ISR_TRA15(void)
{
      while(1);
}
#pragma vector = TIMER1_A0_VECTOR          //14
__interrupt void ISR_TRA14(void)
{
      while(1);
}
#pragma vector = DMA_VECTOR
__interrupt void ISR_TRA13(void)
{
      while(1);
}
#pragma vector = USCI_B2_VECTOR
__interrupt void ISR_TRA12(void)
{
      while(1);
}
#pragma vector = USCI_A2_VECTOR
__interrupt void ISR_TRA11(void)
{
      while(1);
}
#pragma vector = TIMER0_A1_VECTOR      //10
__interrupt void ISR_TRA10(void)
{
      while(1);
}
#pragma vector = TIMER0_A0_VECTOR
__interrupt void ISR_TRAP9(void)
{
      while(1);
}
#pragma vector = ADC12_VECTOR
__interrupt void ISR_TRAP8(void)
{
      while(1);
}
#pragma vector = USCI_B0_VECTOR
__interrupt void ISR_TRAP7(void)
{
...
__interrupt void ISR_TRAP6(void)                 // 6
{
      while(1);
}

#pragma vector = WDT_VECTOR
__interrupt void ISR_TRAP4(void)
{
      while(1);
}

#pragma vector = TIMER0_B1_VECTOR
__interrupt void ISR_TRAP3(void)
{
      while(1);
}
...
__interrupt void ISR_TRAP1(void)                  //2
{
      while(1);
}

#pragma vector =  SYSNMI_VECTOR
__interrupt void ISR_TRAP(void)                   //1
{
      while(1);
}
*/

// lo anteriormente comentado debe ser borrado si anda el programa al final. Es con fines de debug
...
if((P2IFG & BIT2) == BIT2){
   uart_write("Recibimos dato, estamos procesando ISR");
   uart_writec('\r');
   sx1276_on_dio0irq();
